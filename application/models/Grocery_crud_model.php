...
class Grocery_crud_model extends CI_Model {
        ...
        if (! empty($post_data)) {
            foreach ($post_data as $primary_key_value) {
                ...
                if ($count == 0) {
                    ...
                    $this->db->update($field_info->relation_table, array(
                        $field_info->priority_field_relation_table => $counter
                    ), $where_array);
                }
    ...
    function db_update($post_array, $primary_key_value) {
        $primary_key_field = $this->get_primary_key();
        return $this->db->update($this->table_name, $post_array, array(
        ...
        if ($table_name == null) {
            ...
            if (empty($this->primary_key)) {
                ...
                foreach ($fields as $field) {
                    if(isset($field->primary_key) && !empty($field->primary_key)){
                        if ($field->primary_key == 1) return $field->name;
                    }else{
                        if ($field === reset($fields)){
                            $firstArrayKey = $field->primary_key;
                            $firstArrayName = $field->name;
                        }
                        if($firstArrayKey == 0) return $firstArrayName;
                    }
                    /*
