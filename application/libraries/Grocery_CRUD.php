...
class grocery_CRUD_Field_Types {
    ...
    public function get_field_types() {
        ...
        foreach ($this->basic_model->get_field_types_basic_table() as $field_info) {
            ...
            $field_info->display_as = isset($this->display_as[$field_info->name]) ? $this->display_as[$field_info->name] : ucfirst(str_replace("_", " ", $field_info->name));
        ...
        if (! empty($this->relation_n_n)) {
            foreach ($this->relation_n_n as $field_name => $field_extras) {
                ...
                ;
                $field_info->display_as = isset($this->display_as[$field_name]) ? $this->display_as[$field_name] : ucfirst(str_replace("_", " ", $field_name));
        ...
        if (! empty($this->add_fields))
            foreach ($this->add_fields as $field_object) {
                ...
                if (! isset($types[$field_name])) {// Doesn't exist in the database? Create it for the CRUD
                    ...
                    $field_info = (object) array(
                        'name' => $field_name,
                        'crud_type' => $this->change_field_type !== null && isset($this->change_field_type[$field_name]) ? $this->change_field_type[$field_name]->type : 'string',
                        'display_as' => isset($this->display_as[$field_name]) ? $this->display_as[$field_name] : ucfirst(str_replace("_", " ", $field_name)),
...
class grocery_CRUD_Model_Driver extends grocery_CRUD_Field_Types {
    ...
    protected function db_insert_validation() {
         ...
         * If the field value is not unique then
            ...
            foreach ($edit_fields as $edit_field) {
                ...
                if (in_array($field_name, $unique_fields)) {
                    ...
                    if (! isset($row->$field_name)) {
                        throw new Exception("The field name doesn't exist in the database. " . "Please use the unique fields only for fields " . "that exist in the database");
                    }
    ...
    protected function db_multiple_delete($state_info) {
        foreach ($state_info->ids as $delete_id) {
            $result = $this->db_delete((object) array(
                'primary_key' => $delete_id
            ));
            if (! $result) {
                return false;
            }
...
class grocery_CRUD_Layout extends grocery_CRUD_Model_Driver {
    ...
    protected function showList($ajax = false, $state_info = null) {
        ...
        if (! $ajax) {
            $this->_add_js_vars(array(
                'dialog_forms' => $this->config->dialog_forms
            ));
    ...
    protected function _export_to_excel($data) {
        /**
         * No need to use an external library here.
         * The only bad thing without using external library is that Microsoft Excel is complaining
        ...
        foreach ($list as $num_row => $row) {
            ...
            foreach ($this->actions as $unique_id => $action) {
                if (! empty($action->url_callback)) {
                    ...
                    $actions_urls[$unique_id] = $action->url_has_http ? $action->link_url . $row->$primary_key : site_url($action->link_url . '/' . $row->$primary_key);
                }
            }
...
class grocery_CRUD_States extends grocery_CRUD_Layout {
    ...
    protected function state_url($url = '', $is_list_page = false) {
        ...
        if (! empty($this->crud_url_path)) {
            $state_url = ! empty($this->list_url_path) && $is_list_page ? $this->list_url_path : $this->crud_url_path . '/' . $url;
        } else {
...
class Grocery_CRUD extends grocery_CRUD_States {
 ...
 // autogenerate, please do not modify
 ...
 // autogenerate, please do not modify
    ...
    /**
     * Unsets just the jquery library from the js.
     * This function can be used if there is already a jquery included
    ...
    /**
     * Unsets the jquery UI Javascript and CSS.
    ...
    /**
     * Unsets just the twitter bootstrap libraries from the js and css.
     * This function can be used if there is already twitter bootstrap files included
    ...
    /**
     * The fields that user can see.
     * It is only for the add form
     */
    ...
    protected function _load_language() {
        ...
        $this->default_true_false_text = array(
            $this->l('form_inactive'),
            $this->l('form_active')
        );
    ...
    protected function _load_date_format() {
        list ($php_day, $php_month, $php_year) = array(
            'd',
            'm',
            'Y'
        );
        list ($js_day, $js_month, $js_year) = array(
            'dd',
            'mm',
            'yy'
        );
        list ($ui_day, $ui_month, $ui_year) = array(
            $this->l('ui_day'),
            $this->l('ui_month'),
            $this->l('ui_year')
        );
    ...
    protected function get_columns() {
        if ($this->columns_checked === false) {
            ...
            foreach ($this->columns as $col_num => $column) {
                ...
                if (isset($this->display_as[$column]))
                    $this->columns[$col_num] = (object) array(
                        'field_name' => $column,
                        'display_as' => $this->display_as[$column]
                    );
                elseif (isset($field_types[$column]))
                    $this->columns[$col_num] = (object) array(
                        'field_name' => $column,
                        'display_as' => $field_types[$column]->display_as
                    );
                else
                    $this->columns[$col_num] = (object) array(
                        'field_name' => $column,
                        'display_as' => ucfirst(str_replace('_', ' ', $column))
                    );
        ...
        if ($this->read_fields_checked === false) {
            ...
            if (! empty($this->read_fields)) {
                foreach ($this->read_fields as $field_num => $field) {
                    if (isset($this->display_as[$field]))
                        $this->read_fields[$field_num] = (object) array(
                            'field_name' => $field,
                            'display_as' => $this->display_as[$field]
                        );
                    else
    ...
    public function or_like($field, $match = '', $side = 'both', $type = false) {
        $this->or_like[] = array(
            $field,
            $match,
            $side,
    ...
    public function set_relation($field_name, $related_table, $related_title_field, $where_clause = null, $order_by = null) {
        $this->relation[$field_name] = array(
            $field_name,
            $related_table,
            $related_title_field,
            $where_clause,
    ...
    public function set_field_upload($field_name, $upload_dir = '', $allowed_file_types = '') {
        ...
        /**
         * Check if the upload Url folder exists.
...
class UploadHandler{
    ...
    public function post(){
        ...
        if ($upload && is_array($upload['tmp_name'])) {
            ...
            $info[] = $this->handle_file_upload(isset($upload['tmp_name']) ? $upload['tmp_name'] : null, isset($_SERVER['HTTP_X_FILE_NAME']) ? $_SERVER['HTTP_X_FILE_NAME'] : $upload['name'], isset($_SERVER['HTTP_X_FILE_SIZE']) ? $_SERVER['HTTP_X_FILE_SIZE'] : $upload['size'], isset($_SERVER['HTTP_X_FILE_TYPE']) ? $_SERVER['HTTP_X_FILE_TYPE'] : $upload['type'], isset($upload['error']) ? $upload['error'] : null);
        }
        header('Vary: Accept');
